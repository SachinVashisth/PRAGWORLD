...
def extract_entities(S, Index):
    if all(key in S for key in ["<agents>:", "<objects>:", "<locations>:", 
                            "Selected Agents:", "Selected Objects:", "Selected Locations:", 
                            "Generated Conversation:"]):
        
        task_1, task_2, task_3 = re.split(r"Task \d Output:", S)[1:]
        task_1_agent = convert_to_list(re.search(r"<agents>:\s*(.*?)(?=<objects>)", task_1, re.DOTALL).group(1))
        task_1_object = convert_to_list(re.search(r"<objects>:\s*(.*?)(?=<locations>)", task_1, re.DOTALL).group(1))
        task_1_location = convert_to_list(re.search(r"<locations>:\s*(.*)", task_1, re.DOTALL).group(1))

        task_2_selected_agents = convert_to_list(re.search(r"Selected Agents:\s*(.*?)(?=Selected Objects:)", task_2, re.DOTALL).group(1))
        task_2_selected_objects = convert_to_list(re.search(r"Selected Objects:\s*(.*?)(?=Selected Locations:)", task_2, re.DOTALL).group(1))
        task_2_selected_locations = convert_to_list(re.search(r"Selected Locations:\s*(.*?)(?=First Triplet)", task_2, re.DOTALL).group(1))

        task_3_generated_conv = re.search(r"Generated Conversation:\s*(.*)", task_3, re.DOTALL).group(1).strip()
        utterances_task3 = re.findall(r"(Alice: .*?|Bob: .*?)(?=\n\s*|\n\n|$)", task_3_generated_conv, re.DOTALL)
        utterance_list_task3 = [utterance.strip() for utterance in utterances_task3]
        
        result = {
            "Seed Conversation Index": Index,
            "Task 1 Agent": task_1_agent,
            "Task 1 Object": task_1_object,
            "Task 1 Location": task_1_location,
            "Task 2 Selected Agents": task_2_selected_agents,
            "Task 2 Selected Objects": task_2_selected_objects,
...
def perturb_variable_swap(result, Type):
    ...
    if Type == "agent":
        ...
        entity_total, entity_emphasis = result['Task 1 Object'], result['Task 2 Selected Objects']
    elif Type == "location":
...
def perturb_quantifier_change(result):
    ...
    new_result = {}
    predefined_qu_dict = {"all": "some", "some": "all", "All": "Some", "Some": "All"}
    new_result['Seed Conversation Index'] = result['Seed Conversation Index']
    new_result['Task 3 Generated Conversations'] = arrange_sent(result['Task 3 Generated Conversations'], True)
    sentences = result['Task 3 Generated Conversations']
...
def perturb_log_conn_change(result):
    ...
    new_result = {}
    predefined_logconn_dict = {"and": "or", "or": "and", "And": "Or", "Or": "And"}
    new_result['Seed Conversation Index'] = result['Seed Conversation Index']
    new_result['Task 3 Generated Conversations'] = arrange_sent(result['Task 3 Generated Conversations'], True)
    sentences = result['Task 3 Generated Conversations']
...
if __name__ == "__main__":
    ...
    path = "./store_dir/"
    conv_f = [json.loads(lines) for lines in open(path + "grice_unique_conv.json", "r")]
    
    perturb_type = ["Variable Substitution", "Variable Swap", "Quantity Change", "Quantifier Change", "Logical Connective Change"]
    perturb_target = ["agent", "object", "location"]
    count, counter_type_ques = 0, 0
    for i in tqdm(range(len(conv_f[0]))):
