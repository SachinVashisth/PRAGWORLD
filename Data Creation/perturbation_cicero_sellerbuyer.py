...
def extract_entities(S, Index):
    if all(key in S for key in ["<agents>:", "<items>:", "<properties>:", 
                            "Selected Agents:", "Selected Items:", "Selected Properties:", 
                            "Generated Conversation:"]):
        
        task_1, task_2, task_3 = re.split(r"Task \d Output:", S)[1:]
        task_1_agent = [item.lower() for item in convert_to_list(re.search(r"<agents>:\s*(.*?)(?=<items>)", task_1, re.DOTALL).group(1))]
        task_1_object = [item.lower() for item in convert_to_list(re.search(r"<items>:\s*(.*?)(?=<properties>)", task_1, re.DOTALL).group(1))]
        task_1_location = [item.lower() for item in convert_to_list(re.search(r"<properties>:\s*(.*)", task_1, re.DOTALL).group(1))]

        task_2_selected_agents = [item.lower() for item in convert_to_list(re.search(r"Selected Agents:\s*(.*?)(?=Selected Items:)", task_2, re.DOTALL).group(1))]
        task_2_selected_objects = [item.lower() for item in convert_to_list(re.search(r"Selected Items:\s*(.*?)(?=Selected Properties:)", task_2, re.DOTALL).group(1))]
        task_2_selected_locations = [item.lower() for item in convert_to_list(re.search(r"Selected Properties:\s*(.*?)(?=First Triplet)", task_2, re.DOTALL).group(1))]

        task_3_generated_conv = re.search(r"Generated Conversation:\s*(.*)", task_3, re.DOTALL).group(1).strip()
        utterances_task3 = re.findall(r"(A: .*?|B: .*?|Seller: .*?|Buyer: .*?)(?=\n\s*|\n\n|$)", task_3_generated_conv, re.DOTALL)

        utterance_list_task3 = [utterance.strip().lower() for utterance in utterances_task3]
        
        result = {
            "Seed Conversation Index": Index,
            "Task 1 Agents": task_1_agent,
            "Task 1 Items": task_1_object,
            "Task 1 Properties": task_1_location,
            "Task 2 Selected Agents": task_2_selected_agents,
            "Task 2 Selected Items": task_2_selected_objects,
            "Task 2 Selected Properties": task_2_selected_locations,
...
def perturb_variable_swap(result, Type):
    ...
    if Type == "agent":
        entity_total, entity_emphasis = result['Task 1 Agents'], result['Task 2 Selected Agents']
    elif Type == "items":
        entity_total, entity_emphasis = result['Task 1 Items'], result['Task 2 Selected Items']
    elif Type == "properties":
        entity_total, entity_emphasis = result['Task 1 Properties'], result['Task 2 Selected Properties']
...
def perturb_variable_substitution(result, Type):
    ...
    print(entity_external)
    print(entity_emphasis)
    print(entity_external_present)
    print(entity_emphasis_present)
...
if __name__ == "__main__":
    ...
    with open(path + "cicero_sellerbuyer.json", "r", encoding="utf-8") as file:
    ...
    perturb_target = ["items", "properties"]
    ...
    for i in tqdm(range(len(conv_f))):
        ...
        for choosen_perturb_type in perturb_type:
            ...
            if new_result:
                ...
                new_result['Emphasis Entities'] = "Agents: " + " ".join(result['Task 2 Selected Agents']) + "\n" + "Items: " +  " ".join(result['Task 2 Selected Items']) + "\n" + "Properties: " + " ".join(result['Task 2 Selected Properties'])
                
                
                
                with open(path + "cicero_sellerbuyer_perturbed.csv", mode='a', newline='', encoding='utf-8') as csvfile:
